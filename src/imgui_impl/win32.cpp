...
static void ImGui_ImplWin32_UpdateMouseData(SIZE resolution)
{
    ImGui_ImplWin32_Data* bd = ImGui_ImplWin32_GetBackendData();
    ImGuiIO& io = ImGui::GetIO();
    IM_ASSERT(bd->hWnd != 0);
    ...
    if (is_app_focused)
    {
        ...
        if (!io.WantSetMousePos && bd->MouseTrackedArea == 0 && has_mouse_screen_pos)
        {
            ...
            auto xScale = io.DisplaySize.x / (clientRect.right - clientRect.left);
            auto yScale = io.DisplaySize.y / (clientRect.bottom - clientRect.top);
...
void    ImGui_ImplWin32_NewFrame(const SIZE resolution)
{
    ImGuiIO& io = ImGui::GetIO();
    ImGui_ImplWin32_Data* bd = ImGui_ImplWin32_GetBackendData();
    ...
    io.DisplaySize = ImVec2((float)(resolution.cx), (float)(resolution.cy));
    ...
    ImGui_ImplWin32_UpdateMouseData(resolution);
