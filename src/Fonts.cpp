...
const std::tuple<const ImWchar*, std::filesystem::path> Fonts::GetGlyphRange(std::string aLanguage)
{
    ...
    if (aLanguage == "ChineseFull")
        return std::make_tuple(io.Fonts->GetGlyphRangesChineseFull(), GetAbsolutePath(m_defaultFonts["ChineseFull"], m_paths.Fonts(), false));

    // Using GetGlyphRangesChineseFull() instead of GetGlyphRangesChineseSimplifiedCommon(), because the range is not comeplete.
    if (aLanguage == "ChineseSimplifiedCommon")
        return std::make_tuple(io.Fonts->GetGlyphRangesChineseFull(), GetAbsolutePath(m_defaultFonts["ChineseSimplifiedCommon"], m_paths.Fonts(), false));

    if (aLanguage == "Japanese")
        return std::make_tuple(io.Fonts->GetGlyphRangesJapanese(), GetAbsolutePath(m_defaultFonts["Japanese"], m_paths.Fonts(), false));

    if (aLanguage == "Korean")
        return std::make_tuple(io.Fonts->GetGlyphRangesKorean(), GetAbsolutePath(m_defaultFonts["Korean"], m_paths.Fonts(), false));

    if (aLanguage == "Cyrillic")
        return std::make_tuple(io.Fonts->GetGlyphRangesCyrillic(), GetAbsolutePath(m_defaultFonts["Cyrillic"], m_paths.Fonts(), false));

    if (aLanguage == "Thai")
        return std::make_tuple(io.Fonts->GetGlyphRangesThai(), GetAbsolutePath(m_defaultFonts["Thai"], m_paths.Fonts(), false));

    if (aLanguage == "Vietnamese")
        return std::make_tuple(io.Fonts->GetGlyphRangesVietnamese(), GetAbsolutePath(m_defaultFonts["Vietnamese"], m_paths.Fonts(), false));

    // add all glyphs from the font
    if (aLanguage == "Full")
    {
        static const ImWchar range[] = { 0x1, 0xFFFF, 0 };
        return std::make_tuple(range, GetAbsolutePath(m_defaultFonts["Default"], m_paths.Fonts(), false));
    }
    ...
    if (aLanguage == "System")
    {
        ...
        case MAKELANGID(LANG_CHINESE, SUBLANG_CHINESE_SIMPLIFIED):
            return std::make_tuple(io.Fonts->GetGlyphRangesChineseSimplifiedCommon(), GetAbsolutePath(m_defaultFonts["ChineseSimplifiedCommon"], m_paths.Fonts(), false));
    ...
    return std::make_tuple(io.Fonts->GetGlyphRangesDefault(), GetAbsolutePath(m_defaultFonts["Default"], m_paths.Fonts(), false));
}
...
void Fonts::TriggerRebuildFonts()
{
    m_rebuildFonts = true;
}
